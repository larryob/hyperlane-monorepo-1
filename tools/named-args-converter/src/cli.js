...
const argv = yargs(hideBin(process.argv))
  ...
  .example('$0 --dry-run ./contracts/', 'Preview changes in contracts directory')
  .example('$0 --write -m 3 ./contracts/', 'Convert calls with 3+ args and save')
  .example('$0 -v --show-diff ./contracts/Mailbox.sol', 'Show detailed changes for a file')
  .help('h')
  .alias('h', 'help')
  .version('1.0.0')
  .parse();
...
async function main() {
  ...
  if (!targetPath) {
    console.error('Error: Please specify a path to a Solidity file or directory');
    process.exit(1);
  }
    ...
    if (argv.showDiff && summary.totalChanges > 0) {
      ...
      for (const result of summary.results.filter(r => r.changes > 0)) {
        ...
        for (const change of result.details || []) {
          console.log(`Line ${change.loc?.start?.line || '?'}: ${change.funcName}`);
