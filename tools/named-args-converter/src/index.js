...
export class NamedArgsConverter {
  constructor(options = {}) {
    this.options = {
      ...
      excludePatterns: options.excludePatterns || ['**/node_modules/**', '**/lib/**'],
      ...options
    };
  ...
  async convertFile(filePath) {
    ...
    if (this.options.write && !this.options.dryRun && result.changes.length > 0) {
  ...
  async convertDirectory(directory, pattern = '**/*.sol') {
    ...
    console.log(`Registry: ${stats.contracts} contracts, ${stats.functions} functions, ${stats.events} events, ${stats.errors} errors`);
      ...
      for (const result of this.results.filter(r => r.changes > 0)) {
        ...
        if (result.details) {
          for (const change of result.details) {
            console.log(`    - ${change.funcName}() at line ${change.loc?.start?.line || '?'}`);
          }
        }
      }
    }
